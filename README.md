...
│ ├── Dendo_PCA_COSMO_analysis.py # Hierarchical clustering and branch‑and‑bound analysis
...
### 6. Dendo_PCA_COSMO_analysis.py

Performs hierarchical clustering on molecular descriptors (including QTAIM, COSMO, etc.) and uses a branch‑and‑bound (BnB) search on the dendrogram to identify compounds with the best ΔG values while minimising the number of required ΔG measurements (“demaskings”). The script outputs dendrogram plots (colouring only the demasked leaves) and a round‑by‑round assignment file.

**Features:**
- Loads descriptor data from CSV files (supports European decimal format)
- Standardises features and performs hierarchical clustering (default: Ward linkage, Euclidean distance)
- Runs a branch‑and‑bound algorithm over the dendrogram to select compounds for ΔG measurement in rounds
- Generates a dendrogram plot (PNG and SVG) where:
  * Leaves coloured by ΔG on a red‑green scale correspond to compounds that were actually measured
  * Leaves in grey remain unmeasured
- Writes a detailed text file listing, for each round, which compounds were demasked and which clusters were kept

**Usage:**
```bash
python endo_PCA_COSMO_analysis.py examples/COSMO_QTAIM_example_mini.csv <max_depth> <min_branch_size>
```
- ```<max_depth>```: maximum depth of the dendrogram to explore
- ```<min_branch_size>```: minimum cluster size to consider as a valid branch
...
1.  **Understanding Round Assignments**\
  The ```Dendo_PCA_COSMO_analysis.py``` script generates round assignments in a format similar to COSMO_QTAIM_example_mini_rounds_assignment.txt:
  ```bash
Round 1: demasked 1 compound(s): Entry14
  Kept branches: [30]
  Filtered branches: [17]
    Branch 30 members: Entry0, Entry1, Entry2, Entry3,... Entry9, Entry10, Entry11, Entry12, 

Round 2: demasked 1 compound(s): Entry10
  Kept branches: [28]
  Filtered branches: [29]
    Branch 28 members: Entry10, Entry11
  ...
  ```
  Key points:
  * Compounds are selected for measurement in rounds to minimise the number of ΔG measurements while aiming to find the best compound
...
- Additional files:
  ...
  * ```COSMO_QTAIM_example_mini_dendrogram.png / COSMO_QTAIM_example_mini_dendrogram.svg```: typical dendrogram visualisation from the hierarchical clustering analysis, with leaves coloured by ΔG and only demasked compounds highlighted.
  * ```COSMO_QTAIM_example_mini_rounds_assignment.txt```:  corresponding file showing the assigned *Rounds* distribution from the branch‑and‑bound search.
...
- MOPAC: Download from [MOPAC official site](https://openmopac.net/)
